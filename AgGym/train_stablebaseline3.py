...
class EpisodicRewardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.locals.get('done'):  # Check if episode is done
            info = self.locals.get('self.info')  # Retrieve info dictionary
...
if env.model == 'ddqn':
    ...
    tmp_path = f"{cwd}/newresults/{now_str}/data/config/"
    new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])
    tensorboard_log="/data/mahsak/AgGym2/tensorboard/"
    agent = PPO(policy = "MlpPolicy", env = env, learning_rate=0.0001, gamma=0.9, tensorboard_log=tensorboard_log, policy_kwargs=policy_kwargs, verbose=1)

    eval_callback = EvalCallback(env, best_model_save_path=cwd / 'newresults' / now_str / 'data' / 'model_log',
                            log_path=cwd / 'newresults' / now_str / 'data' / 'log',  eval_freq=1000,
                            deterministic=True, render=False)
    # agent.set_logger(new_logger)
    # agent = PPO("MlpPolicy", env, learning_rate=0.001, n_steps=128, ent_coef=0.01, gamma=0.9, gae_lambda=0.95, n_epochs=10, verbose=1, policy_kwargs=policy_kwargs)
    agent.learn(total_timesteps=max_episode, callback= eval_callback)
    # agent = PPO(CustomPolicy, env, verbose=1)
    # print(callback.episode_rewards)
    # training_R = callback.episode_rewards
    # print(training_R)
...
# )
    ...
    policy_kwargs = dict(
    net_arch=[512, 256, 128]  # Three hidden layers each with 256 neurons
)
    tensorboard_log="/data/mahsak/AgGym2/tensorboard/"
    eval_callback = EvalCallback(env, best_model_save_path=cwd / 'newresults' / now_str / 'data' / 'model_log',
                            log_path=cwd / 'newresults' / now_str / 'data' / 'log',  eval_freq=1000,
    ...
    gamma=0.9, tensorboard_log=tensorboard_log, verbose=1)
    max_episode = config["training"].as_int("max_episode") 
    # agent = TRPO("MlpPolicy", env, verbose=1)
    agent.learn(total_timesteps= max_episode, log_interval=4, callback= eval_callback)
    ...
    eval_callback = EvalCallback(env, best_model_save_path=cwd / 'newresults' / now_str / 'data' / 'model_log',
                            log_path=cwd / 'newresults' / now_str / 'data' / 'log',  eval_freq=1000,
                            deterministic=True, render=False)
    agent = A2C("MlpPolicy", env, learning_rate=0.0001, 
    policy_kwargs=policy_kwargs,
    gamma=0.9, tensorboard_log=tensorboard_log, verbose=1)
    agent.learn(total_timesteps=max_episode, callback= eval_callback)
elif env.model == 'sac':
...
eval_reward = []
print('eval_mode****************************************')
best_model_path=cwd / 'newresults' / now_str / 'data' / 'model_log'/'best_model.zip'

agent = TRPO.load(best_model_path, env=env, verbose=1)
obs, info = env.reset()
done = False
R = 0
...
eval_reward.append(R)
print(f"Evaluation Episode:, Reward: {R}")
print()
logging.info(f"Evaluation Episode:, Reward: {R}")
...
#     print(f"Evaluation Episode: {eval_ep}, Reward: {R}")
#     logging.info(f"Evaluation Episode: {eval_ep}, Reward: {R}")
#here is the initial evaluation and should be uncommented once the sbase3 evaluation is figured out
#     training_R.append(R)
#     if ep % 100 == 0:

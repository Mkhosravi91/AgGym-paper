...
import numpy as np
from utils import env_utils_granular
from modules import threat_modules_pestval_granular as tm
...
class cartesian_grid(gym.Env):
    ...
    def init(self, size=None, multigrid=None, multi_coord=None, measurements=None):
        # self.observation_space = Discrete(len(self.coords_x))
        ...
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(len(self.coords_x),), dtype=np.float32)
        self.action_space = MultiDiscrete([4]*len(self.coords_x)) 
    ...
    def rl_init(self):
        ...
        self.reward_accumulator=reward_accumulator.init(self)
        action_delegator.init(self)
        state_accumulator.init(self)
        done_delegator.init(self)

        env_utils_granular.set_growth_stage_dict(self)
        env_utils_granular.retrieve_growth_stage(self)
        if self.mode == "eval":
            env_utils_granular.plot_field(self, "initial_grid")

        logging.info("---------- Environment Parameters ----------")
        # logging.info(f"Action Space: {self.action_space.n}")
        logging.info(f"Action Space: {self.action_space.nvec}")
    ...
    def step(self, action):
        ...
        # self.pesticide_rewards.append(self.reward_accumulator.pesticide_rewards)
        # self.yiled_rewards.append(self.reward_accumulator.yiled_rewards)
        # print(self.reward_accumulator.pesticide_rewards)
        # print(self.reward_accumulator.yiled_rewards)
        # print(self.pesticide_rewards)
        # print(self.yiled_rewards)
        # self.episode_total_reward +=reward
        # if done:
        #     self.info = {'episode_reward': self.episode_total_reward}
        # else:
        #     self.info = {}
        ...
        if self.mode == "eval" or (self.mode == "train" and self.episode % 11500 == 0 and self.plot_progression == 'True'):
             # or (self.mode == "train" and self.episode % 1000 == 0)
            #  self.mode == "train" and self.episode % int(self.max_episode) == 0
            env_utils_granular.plot_field(self, f"step_plot_state_{str(self.timestep).zfill(6)}")
        self.timestep += 1
        
        # return state_accumulator.main(self), reward, done, truncated, self.info
        return state_accumulator.main(self), reward, done, truncated, {}
    def render(self, ax, val):
        ...
        for (y, x), value in self.threat.action_map.items():
            ...
            object_color = 'green'
            object_size = 5  # choose a size for your object
            if value != 0:  # check if the value is not zero
                rectangle = plt.Rectangle((x-2, y-2), 4, 4, fc=object_color)
                ax[val].add_patch(rectangle)
        # if self.timestep==2: 
        #     pdb.set_trace()
        if self.multi_lvl:

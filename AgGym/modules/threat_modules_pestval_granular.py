...
class Threat_basic:
    ...
    def reshape_to_fixed_matrix(self, vec):
        matrix = np.zeros((10, 10))
        matrix.flat[:len(vec)] = vec
        return matrix
    ...
    def apply_pesticide(self, action: float):
        ...
        print(action)
        print(type(action))
        self.action_matrix=self.reshape_to_fixed_matrix(action)
        ...
        for idx, act in enumerate(action):
            self.coord = (self.coords_y[idx], self.coords_x[idx])
            self.action_map[self.coord] = act
        ...
        self.healthy=[]
        # idx = 0
        for x in range(len(self.coords_x)):
            for y in range(len(self.coords_y)):
                if x >= self.action_matrix.shape[0] or j >= self.action_matrix.shape[1]:
                    continue
                # self.action_value = action_matrix[x, y]
                # print(self.action_value)
                # print(self.infect_list)
        for coord, value in self.action_map.items():
            if coord not in self.infect_list and value != 0:
                self.alive_sprayed.append(coord)
            
            # Get the chance if the current cell is infected
        
        for idx, k in enumerate(self.infect_list):
            x, y = k
            # self.action_value = self.action_matrix[x//10, y//10]
            # print(self.action_matrix[x//10, y//10])
            self.action_value= self.action_map.get((x, y))
            if (x, y) not in self.infect_sprayed and self.action_value != 0:
                self.infect_sprayed.append((x, y))
            pesticide_chance = self.pesticide_actions[self.action_value]
            pest_chance = np.random.choice([0, 1], p=[1-pesticide_chance, pesticide_chance])
        
            if pest_chance == 1:
                if self.grid[x, y] != DEAD:
                    self.grid[x, y] = ALIVE
                    self.infect_day_mat[x, y] = 0.
                    temp_list.append(idx)

        for k in temp_list[::-1]:
            self.infect_list.pop(k)
        for x in self.coords_x:
            for y in self.coords_y:
                coord = (x, y)
            # Check if the coordinate meets the given conditions
                if coord not in self.infect_list and coord not in self.alive_sprayed and coord not in self.infect_sprayed:
                    self.healthy.append(coord)
    ...
    def spread_probabilities_sprayed(self):
        ...
        low_infect = np.random.choice([0,1], size=24, replace=True, p=[0.95, 0.05])
        self.infection_sprayed = {'high': high_infect, 'medium': medium_infect, 'low': low_infect}
    ...
    def check_neighbour(self):
        ...
        for plant_infected in temp_list:
            ...
            for (i, j) in zip(neighbouring_idx[0], neighbouring_idx[1]):
                if (i, j) == (x, y):
                    continue
                if (j, i) not in self.infect_list and self.grid[j, i] != DEAD and (j, i) not in self.alive_sprayed:
                ...
                elif (j, i) not in self.infect_list and self.grid[j, i] != DEAD and (j, i) in self.alive_sprayed:
                    if (j, i) not in self.infect_sprayed and self.infection_sprayed['high'][idx] == 1:
                            self.grid[j, i] = INFECTED
                            self.infect_list.append((j,i))
                    elif (j, i) in self.infect_sprayed and self.infection_sprayed['high'][idx] == 2:
                            self.grid[j, i] = INFECTED
                            self.infect_list.append((j,i))
                idx += 1
            end = time.time()
            ...
            for i, j in zip(neighbouring_idx[0], neighbouring_idx[1]):
                ...
                if (j, i) not in self.infect_list and self.grid[j, i] != DEAD and (j, i) not in self.alive_sprayed:
                    ...
                    if (j, i) not in self.infect_sprayed and self.infection_sprayed['medium'][idx] == 1:
                            self.grid[j, i] = INFECTED
                            self.infect_list.append((j,i))
                    elif (j, i) in self.infect_sprayed and self.infection_sprayed['medium'][idx] == 2:
                            self.grid[j, i] = INFECTED
                            self.infect_list.append((j,i))
                idx += 1
            end = time.time()

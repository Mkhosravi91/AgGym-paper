...
def plot_grid(self, title):
    ...
    if self.mode == "eval":
        ...
        (Path(self.result_path) / 'eval' / f'agent_').mkdir(parents=True, exist_ok=True)
        # plt.savefig(Path(self.result_path) / 'eval' / f'agent_{self.best_agent}' / f"{title}.pdf", dpi=300)
        plt.savefig(Path(self.result_path) / 'eval' / f'agent_' / f"{title}.pdf", dpi=300)
    elif self.mode == "train" and self.episode % 1000 == 0:
        (Path(self.result_path) / f"ep_{self.episode}").mkdir(parents=True, exist_ok=True)
        plt.savefig(Path(self.result_path) / f"ep_{self.episode}" / f"{title}.pdf", dpi=300)
    plt.close()
...
def plot_field(self, title):
    ...
    sns.lineplot(x=np.arange(len(self.reward_list)), y=self.reward_list, ax=ax_dict["E"])
    ax_dict["E"].spines[:].set_visible(False)
    ax_dict["E"].tick_params(axis="both", which="both", bottom=False, left=False)
    ax_dict["E"].set_ylabel("Reward")
    ax_dict["E"].set_xlabel("Planting Days")
    ax_dict["E"].set_title("Reward Earned Across One Trajectory")
    ...
    if self.mode == "eval":
        ...
        plt.savefig(Path(self.result_path) / 'eval' / f'agent_' / f"{title}.pdf", dpi=300)
    elif self.mode == "train" and self.episode % 11500 == 0 and self.plot_progression == 'True':
        (Path(self.result_path) / f"ep_{self.episode}").mkdir(parents=True, exist_ok=True)
        plt.savefig(Path(self.result_path) / f"ep_{self.episode}" / f"{title}.pdf", dpi=300)
    plt.close()

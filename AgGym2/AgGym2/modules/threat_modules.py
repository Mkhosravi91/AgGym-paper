...
class Threat_basic:
    ...
    def spread_probabilities(self):
        high_infect = np.random.choice([0,1], size=8, replace=True, p=[0.7, 0.3])
        medium_infect = np.random.choice([0,1], size=16, replace=True, p=[0.85, 0.15])
    ...
    def compute_infection(self, action, grid, gs_title, sim_mode, severity):
        self.gs_title = gs_title
        self.grid = grid
        if sim_mode == 'growthseason':
            start = time.time()

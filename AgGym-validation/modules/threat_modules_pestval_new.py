...
class Threat:
    ...
    def apply_infection_reproductive_stage_survival(self):
        ...
        self.gen_dead_count = dead_count
...
class Threat_basic:
    ...
    def apply_infection_reproductive_stage(self):
        #if self.gs_title.find('StartFlowering') != -1:
            #pdb.set_trace()
    ...
    #     pesticide_chance = self.pesticide_actions[action]
    #     temp_list = []
    #     pest_chance_list = np.random.choice([0,1], size=(len(self.grid)*len(self.grid[0])), replace=True, p=[1-pesticide_chance, pesticide_chance])
    #     idx = 0
    #     print(self.grid.shape)
    #     for p, i in zip(pest_chance_list, grid_list):
    #         if p == 1:
    #             y, x = i
    #             # if not dead yet, cure it
    ...
    #                 try:
    #                     idx=self.infect_list.index((y, x))
    ...
    def spread_probabilities(self):
        high_infect = np.random.choice([0,1], size=8, replace=True, p=[0.65, 0.35])
        medium_infect = np.random.choice([0,1], size=16, replace=True, p=[0.8, 0.2])
    ...
    def compute_infection(self, action, grid, gs_title,timestep, sim_mode, severity):
        self.gs_title = gs_title
        self.grid = grid
        self.timestep=timestep
        if sim_mode == 'growthseason':
        ...
        return self.grid, self.Degraded_list

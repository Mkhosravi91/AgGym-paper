...
class Threat:
    def __init__(self, threat_type: str, severity: int, plot_size: Tuple[int, int]):
    ...
    def compute_infection(self, plot_state, gs_title, sim_mode):
...
class Threat_basic:
    def __init__(self, w: int, h: int, ow: int, oh: int, 
                 grid: np.ndarray, coords_x: List[float], coords_y: List[float], infect_list):
    ...
    def spread_probabilities(self):
        ...
        medium_infect = np.random.choice([0,1], size=16, replace=True, p=[0.70, 0.30])
    ...
    def compute_infection(self, action, grid, gs_title, sim_mode):
        self.gs_title = gs_title
        self.grid = grid
        if sim_mode == 'growthseason':
            start = time.time()

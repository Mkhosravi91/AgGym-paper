...
np.save(cwd / 'results' / now_str / 'data' / 'training_reward', training_R)
...
with agent.eval_mode():
    for ep in range(1):
        ...
        while True:
            ...
            # print(f' precipitation list is {env.precipitation_list[0][env.timestep]}')
            # print(f' precipitation list is {env.precipitation_list}')
            # print(f' templist is {env.temp_list}')
            total_step+=1
            if done == True:
                break
        eval_reward.append(R)

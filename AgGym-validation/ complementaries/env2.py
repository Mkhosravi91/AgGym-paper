...
from modules import new_threat_modules as tm
...
class cartesian_grid:
    ...
    def rl_init(self):
        ...
        self.threat = eval(f"tm.{self.threat_name}({self.plotL*2}, {self.plotW*2}, {self.plotL*4}, {self.plotW*4}, self.grid, self.coords_x, self.coords_y)")
    ...
    def step(self):
        # self.action = action
        start = time.time()
        self.grid = self.threat.compute_infection(self.grid, self.gs_title, self.sim_mode)
        end = time.time()
        ...
        return state_accumulator.main(self), done

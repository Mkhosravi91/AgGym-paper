...
class Threat_basic:
    ...
    def spread_probabilities(self):
        high_infect = np.random.choice([0,1], size=25, replace=True, p=[0.95, 0.05])
        medium_infect = np.random.choice([0,1], size=39, replace=True, p=[0.7, 0.3])
...
class env:
    def __init__(self):
      ...
      df = pd.read_csv("/home/mahsak/AgGym-validation/bailey_farm_YieldNA_removed_row_column.csv")
      df['Mean_20210911'].replace(np.nan, 0, inplace=True)
      self.coords_x= df['Row']
      self.coords_y= df['Column'] 
      grid=np.zeros((137, 180))
      for i in range(len(df['Mean_20210911'])):
        grid[(df.iloc[i, 50]-1), (df.iloc[i, 51]-1)]=df.iloc[i, 37] 
      grid[(grid>0) & (grid <=0.7)] = 0.3    
      self.grid=grid
      
      inxy=np.where(df['Mean_20210911']<=0.76)
      self.infect_list =[]
      for i in inxy[0]:
          self.infect_list.append((df.iloc[i, 50]-1, df.iloc[i, 51]-1))
    ...
    def step(self, action):
        ...
        # print(self.grid)
        return self.grid
df2 = pd.read_csv("/home/mahsak/AgGym-validation/bailey_farm_YieldNA_removed_row_column.csv")
print("h1")
df2.replace(np.nan, 0, inplace=True)
list_name=[]
for col in df2.columns[37:49]:
    list_name.append(col)


###ALI's CHANGES With Env
# env
df2 = pd.read_csv("/home/mahsak/AgGym-validation/bailey_farm_YieldNA_removed_row_column.csv")
df2.replace(np.nan, 0, inplace=True)
...
name_list = ['Mean_20210911', 'Mean_20210919', 'Mean_20210922', 'Mean_20210924']
for k in range(12):
  Env.step(action=0)
  ...
  for i, j in zip(inx, iny):
    ...
    for l in range(len(col)):
      grid_orig[(df2.iloc[l, 50]-1), (df2.iloc[l, 51]-1)]=col[l]
    Env.grid[(i, j)]=grid_orig[(i, j)]

#   plt.imshow(Env.grid, cmap='Greens')
  g18_min, g18_max = np.abs(Env.grid).min(), np.abs(Env.grid).max()
  c=plt.imshow(Env.grid, cmap='Greens',vmin = g18_min, vmax = g18_max)
  plt.colorbar(c)
  
  plt.savefig(f'fig_{k}.png', format='png', dpi = 400)
